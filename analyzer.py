...
class Parser(object):
    ...
    def parse(self, filepath):
        ...
        It returns a iterable having this structure:
        {
            Object_ID:
            {
        ...
        for matchObj in regex.finditer(data):
            match = (matchObj[1], matchObj[2], matchObj[3], matchObj[4])
            try:
                self._parse_lines(match[3])
                yield (int(match[0]), {"ClassID": int(match[1]), "Type": match[2], "Content": self._parse_obj()})
                # objects[int(match[0])] = {"ClassID": int(match[1]), "Type": match[2], "Content": self._parse_obj()}
            except Exception as e:
                print("Error in " + str(matchObj.span()))
                self._print_error()
                raise
